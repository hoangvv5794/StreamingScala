...
import com.viettel.vtcc.spark.model.{StorageCount, StorageStock, TelegramObject}
...
object StreamTelegram extends Serializable {
  def main(args: Array[String]): Unit = {
    ...
    //compute number process message
  ...
  def addCurrentTimeToObject(x: TelegramObject): TelegramObject = {
    ...
    TelegramObject(id = x.id, date = current_date, text = x.text, from = x.from)
  }
